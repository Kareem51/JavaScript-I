...
for(let i = 0; i < inventory.length; i++){
  if(inventory[i].id == 49){
    console.log(`The last car is a ${inventory[i].car_model} ${inventory[i].car_make}`)
  }
}
...
carModels = [];
for(let i = 0; i < inventory.length; i++){
  carModels.push(inventory[i].car_model.toString()); 
}
...
let BMWAndAudi = [];
let bAndA = inventory.map(function (e){

      if ((e.car_make ==="BMW" || e.car_make === "Audi")){
        BMWAndAudi.push(`${e.car_model} ${e.car_make}`)
    }


    })

console.log(BMWAndAudi.toString());
